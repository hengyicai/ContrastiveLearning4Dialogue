...
from parlai.utils.misc import Timer, nice_report, warn_once
...
class TrainLoop:
    ...
    def __init__(self, opt):
        ...
        self.test_worlds = None  # added by HengyiCai
        self.opt = opt
    ...
    def train(self):
                ...
                if (
                    ...
                    if opt.get('run_test_after_validation', False):
                        ...
                        if self.test_worlds is None:
                            # we need to load the world now
                            self.test_worlds = load_eval_worlds(self.agent, test_opt, 'test')
                        run_eval(self.test_worlds, test_opt, 'test', -1, write_log=True)
                    # --------------- change by hengyicai -------------------------
                    if stop_training:
                        break
        ...
        if os.path.isfile(last_model):
            ...
            valid_worlds = load_eval_worlds(last_agent, last_model_opt, 'valid')
            max_exs = last_model_opt['validation_max_exs'] if last_model_opt.get('short_final_eval') else -1
            run_eval(valid_worlds, last_model_opt, 'valid', max_exs, write_log=True)
            test_worlds = load_eval_worlds(last_agent, last_model_opt, 'test')
            run_eval(test_worlds, last_model_opt, 'test', max_exs, write_log=True)
            if valid_worlds:
                for valid_world in valid_worlds:
                    valid_world.shutdown()
            if test_worlds:
